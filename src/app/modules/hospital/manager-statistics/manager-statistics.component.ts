...
export class ManagerStatisticsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.statService.getStats().subscribe(res => {
      ...
      this.stats = res;
        ...
        // statisticsList[0] = number of steps while scheduling and their occurences -> koliko puta se u 4 koraka koliko puta u 6..
        // statisticsList[1] = time spent on each step (in seconds) and their occurences -> tabela?
        // statisticsList[2] = how many times next was clicked in one session and occurences -> 6 korisnika je 4 puta stisnulo next u jednoj sesiji...
  ...
  chart1Options = {
    ...
    zoomEnabled: true,
    title: {
      text: "Number of steps while scheduling"
    },
    axisY: {
